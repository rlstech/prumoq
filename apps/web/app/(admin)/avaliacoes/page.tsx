...
export default async function EvaluationsPage({ searchParams }: { searchParams?: { page?: string | string[] } }) {
  ...
  const [{ data: evaluations }, { data: models }, { data: revisions }, { data: criteria }, { data: works }, { data: teams }, { data: companies }, { data: users }] = await Promise.all([
  ...
  return <><Header breadcrumbs={[{ label: 'Avaliações' }]} /><main className="prumo-page"><div className="prumo-page-inner"><EvaluationsClient evaluations={evaluationPage.rows} models={models ?? []} revisions={revisions ?? []} criteria={criteria ?? []} works={works ?? []} teams={teams ?? []} companies={companies ?? []} users={users ?? []} canManage={context?.perfil === 'admin'} page={page} hasNextPage={evaluationPage.hasNextPage} /></div></main></>;
}
