import { useQuery } from '@powersync/react-native';
import { useRouter } from 'expo-router';
import { ChevronRight, ClipboardCheck, FileDown, Plus } from 'lucide-react-native';
import { Pressable, SafeAreaView, ScrollView, StyleSheet, Text, View } from 'react-native';
import { AppHeader } from '../../../../components/AppHeader';
import { Badge, type BadgeTone, Button, EmptyState, ListSurface, OperationalRow } from '../../../../components/ui';
import { Breakpoints, Colors, FontFamily, FontSizes, Radius, Spacing, Typography } from '../../../../lib/constants';
import { formatDateOnly } from '../../../../lib/verification-detail';

type Row = { id:string; obra_id:string; equipe_id:string; medicao_id:string|null; data_avaliacao:string; status:string; percentual:string; obra_nome:string; equipe_nome:string; referencia:string|null };
type PendingRow = { id:string; referencia:string; obra_nome:string; equipe_nome:string };

// As strings SQL abaixo são reconhecidas por pattern-matching em
// lib/powersync-web-shim.ts para traduzir a query em chamadas Supabase no PWA.
// Alterá-las quebra a versão web silenciosamente.
const QUERY = `SELECT a.id,a.obra_id,a.equipe_id,a.medicao_id,a.data_avaliacao,a.status,a.percentual,o.nome obra_nome,e.nome equipe_nome,m.referencia FROM avaliacoes_empreiteiro a JOIN obras o ON o.id=a.obra_id JOIN equipes e ON e.id=a.equipe_id LEFT JOIN medicoes_servico m ON m.id=a.medicao_id ORDER BY a.created_at DESC`;
const PENDING = `SELECT m.id,m.referencia,o.nome obra_nome,e.nome equipe_nome FROM medicoes_servico m JOIN obras o ON o.id=m.obra_id JOIN equipes e ON e.id=m.equipe_id LEFT JOIN avaliacoes_empreiteiro a ON a.medicao_id=m.id AND a.status IN ('rascunho','concluida','aprovada') WHERE m.status='rascunho' AND e.tipo='terceirizado' AND a.id IS NULL ORDER BY m.data_medicao DESC`;

const STATUS_META: Record<string, { label: string; tone: BadgeTone }> = {
  aprovada: { label: 'Aprovada', tone: 'success' },
  concluida: { label: 'Aguardando aprovação', tone: 'warning' },
  aguardando_aprovacao: { label: 'Aguardando aprovação', tone: 'warning' },
  invalidada: { label: 'Invalidada', tone: 'neutral' },
  rascunho: { label: 'Rascunho', tone: 'neutral' },
};

function statusMeta(status: string): { label: string; tone: BadgeTone } {
  return STATUS_META[status] ?? { label: status.replaceAll('_', ' '), tone: 'neutral' };
}

function toneColor(tone: BadgeTone): string {
  if (tone === 'danger') return Colors.nok;
  if (tone === 'warning') return Colors.warn;
  if (tone === 'success') return Colors.ok;
  if (tone === 'info') return Colors.info;
  return Colors.textSecondary;
}

const isEditable = (status:string) => status === 'rascunho' || status === 'concluida';
const canExport = (status:string) => status === 'concluida' || status === 'aprovada' || status === 'invalidada';
const hasScore = (status:string) => status !== 'rascunho';

export default function EvaluationsScreen() {
  const router = useRouter();
  const { data: items, isLoading } = useQuery<Row>(QUERY, []);
  const { data: pending } = useQuery<PendingRow>(PENDING, []);

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Avaliações" subtitle="Fornecedores de serviço" />
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <View>
            <Text style={styles.heroTitle}>Avaliações em campo</Text>
            <Text style={styles.heroCopy}>
              {pending.length} {pending.length === 1 ? 'medição aguardando avaliação' : 'medições aguardando avaliação'}
            </Text>
          </View>
          <View style={styles.heroBadge}>
            <ClipboardCheck size={22} color={Colors.brand} />
          </View>
        </View>

        {pending.length ? (
          <View style={styles.section}>
            <Text style={styles.title}>Aguardando avaliação</Text>
            <ListSurface>
              {pending.map((item, index) => (
                <PendingRowItem
                  key={item.id}
                  item={item}
                  last={index === pending.length - 1}
                  onOpen={() => router.push(`/avaliacoes/nova?medicaoId=${item.id}` as never)}
                />
              ))}
            </ListSurface>
          </View>
        ) : null}

        <View style={styles.section}>
          <View style={styles.heading}>
            <Text style={styles.title}>Histórico</Text>
            <Button
              label="Avulsa"
              Icon={Plus}
              variant="secondary"
              onPress={() => router.push('/avaliacoes/nova' as never)}
            />
          </View>
          {isLoading ? (
            <Text style={styles.loadingText}>Carregando avaliações...</Text>
          ) : items.length ? (
            <ListSurface>
              {items.map((item, index) => (
                <EvaluationRowItem
                  key={item.id}
                  item={item}
                  last={index === items.length - 1}
                  onOpen={() => {
                    if (isEditable(item.status)) {
                      router.push(`/avaliacoes/nova?avaliacaoId=${item.id}` as never);
                    }
                  }}
                  onExport={() => router.push(`/avaliacoes/${item.id}/pdf` as never)}
                />
              ))}
            </ListSurface>
          ) : (
            <EmptyState
              Icon={ClipboardCheck}
              title="Nenhuma avaliação registrada"
              description="As avaliações concluídas no campo aparecerão aqui."
            />
          )}
        </View>
      </ScrollView>
    </SafeAreaView>
  );
}

function PendingRowItem({
  item,
  last,
  onOpen,
}: {
  item: PendingRow;
  last: boolean;
  onOpen: () => void;
}) {
  return (
    <OperationalRow
      tone="info"
      last={last}
      onPress={onOpen}
      accessibilityLabel={`Avaliar medição ${item.referencia}`}
      trailing={<ChevronRight size={19} color={Colors.textTertiary} />}
    >
      <View style={styles.rowTop}>
        <Text style={styles.rowTitle} numberOfLines={1}>{item.referencia}</Text>
      </View>
      <Text style={styles.rowContext} numberOfLines={1}>
        {item.equipe_nome} · {item.obra_nome}
      </Text>
    </OperationalRow>
  );
}

function EvaluationRowItem({
  item,
  last,
  onOpen,
  onExport,
}: {
  item: Row;
  last: boolean;
  onOpen: () => void;
  onExport: () => void;
}) {
  const status = statusMeta(item.status);
  const percentual = Number(item.percentual);
  const score = hasScore(item.status) && Number.isFinite(percentual)
    ? `${percentual.toFixed(0)}%`
    : 'Sem nota';

  return (
    <OperationalRow
      tone={status.tone}
      last={last}
      onPress={onOpen}
      accessibilityLabel={
        `${isEditable(item.status) ? 'Editar avaliação' : 'Avaliação'} de ${item.equipe_nome}, ` +
        `${item.obra_nome}, ${status.label}, ${score}.`
      }
      trailing={
        canExport(item.status) ? (
          <Pressable
            onPress={onExport}
            style={styles.pdfButton}
            accessibilityRole="button"
            accessibilityLabel={`Gerar PDF da avaliação de ${item.equipe_nome}`}
          >
            <FileDown size={16} color={Colors.brand} />
          </Pressable>
        ) : (
          <ChevronRight size={19} color={Colors.textTertiary} />
        )
      }
    >
      <View style={styles.rowTop}>
        <Text style={styles.rowTitle} numberOfLines={1}>{item.equipe_nome}</Text>
        <Badge label={status.label} tone={status.tone} size="sm" />
      </View>
      <Text style={styles.rowContext} numberOfLines={1}>
        {item.obra_nome} · {item.referencia ?? 'Avulsa'}
      </Text>
      <View style={styles.rowFooter}>
        <Text style={[styles.footerLabel, { color: toneColor(status.tone) }]}>{score}</Text>
        <Text style={styles.footerDate} numberOfLines={1}>{formatDateOnly(item.data_avaliacao)}</Text>
      </View>
    </OperationalRow>
  );
}

const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: Colors.bg },
  content: {
    width: '100%',
    maxWidth: Breakpoints.maxContent,
    alignSelf: 'center',
    padding: Spacing.lg,
    paddingBottom: 110,
    gap: Spacing.xl,
  },
  hero: {
    backgroundColor: Colors.surface,
    borderWidth: 1,
    borderColor: Colors.border,
    borderRadius: Radius.lg,
    padding: Spacing.lg,
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  },
  heroTitle: { ...Typography.heading, color: Colors.text },
  heroCopy: { ...Typography.caption, color: Colors.textSecondary, marginTop: 4 },
  heroBadge: {
    width: 46,
    height: 46,
    borderRadius: Radius.md,
    backgroundColor: Colors.brandLight,
    alignItems: 'center',
    justifyContent: 'center',
  },
  section: { gap: Spacing.sm },
  heading: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  title: { ...Typography.heading, color: Colors.text },
  loadingText: { ...Typography.caption, color: Colors.textTertiary },

  rowTop: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm },
  rowTitle: {
    flex: 1,
    fontFamily: FontFamily.semibold,
    fontSize: FontSizes.base,
    color: Colors.text,
  },
  rowContext: {
    marginTop: 3,
    fontFamily: FontFamily.medium,
    fontSize: FontSizes.sm,
    color: Colors.textSecondary,
  },
  rowFooter: {
    marginTop: Spacing.sm,
    flexDirection: 'row',
    alignItems: 'center',
    gap: Spacing.md,
  },
  footerLabel: {
    fontFamily: FontFamily.semibold,
    fontSize: FontSizes.tiny,
    textTransform: 'uppercase',
    letterSpacing: 0.35,
  },
  footerDate: {
    flex: 1,
    textAlign: 'right',
    fontFamily: FontFamily.monoSemibold,
    fontSize: FontSizes.tiny,
    color: Colors.textTertiary,
  },
  pdfButton: {
    minWidth: 44,
    minHeight: 44,
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: Radius.md,
    backgroundColor: Colors.brandLight,
  },
});
