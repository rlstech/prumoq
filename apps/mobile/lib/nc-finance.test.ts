...
test('declaração financeira de NC exige os campos da situação escolhida', () => {
  ...
  assert.equal(validateNcFinancialDeclaration({ situacao: 'confirmado', bloqueio: 'parcial', valorConfirmado: '10', responsavelFinanceiro: 'empreiteiro', categoria: 'outro' }), 'Bloqueio parcial exige valor bloqueado positivo.');
  ...
  assert.equal(validateNcFinancialDeclaration({ situacao: 'confirmado', bloqueio: 'parcial', valorConfirmado: '100', responsavelFinanceiro: 'empreiteiro', categoria: 'mao_obra_retrabalho', valorBloqueado: '200' }), null);
});
