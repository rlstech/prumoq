-- Reúne todas as evidências fotográficas de uma FVS para os relatórios.
-- A função é SECURITY DEFINER para evitar RLS aninhado nas tabelas de NC,
-- mas valida explicitamente o usuário e o acesso à obra antes de retornar dados.
  ...
  WHERE nc.foto_reinspecao_url IS NOT NULL
    AND NOT EXISTS (
      ...
      WHERE nr.nc_id = nc.id
        AND nr.foto_url = nc.foto_reinspecao_url
    )
