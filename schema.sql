...
create table obras (
  ...
  nome                text not null,
  tipo                text, -- 'residencial', 'comercial', 'industrial', etc.
  endereco            text,
  municipio           text,
  uf                  char(2),
  cep                 text,
...
-- Acesso de usuários por obra (inspetor/gestor → obra)
create table obra_usuarios (
  id          uuid primary key default uuid_generate_v4(),
  obra_id     uuid not null references obras(id) on delete cascade,
  usuario_id  uuid not null references usuarios(id) on delete cascade,
  papel       text not null default 'inspetor', -- 'inspetor', 'gestor', 'fiscal'
  ativo       boolean not null default true,
  created_at  timestamptz not null default now(),
...
create table equipes (
  ...
  responsavel     text,
  telefone        text,
  cnpj_terceiro   text, -- apenas para terceirizados
  especialidade   text,
  ativo           boolean not null default true,
  created_at      timestamptz not null default now(),
  updated_at      timestamptz not null default now()
...
-- ============================================================
-- FVS PADRÃO (biblioteca de fichas da empresa)
-- ============================================================
create table fvs_padrao (
  ...
  norma_ref       text, -- ex: 'ABNT NBR 7200:2019'
...
create table fvs_padrao_revisoes (
  ...
  descricao_alt   text not null, -- o que mudou nesta revisão
  revisado_por    uuid references usuarios(id) on delete set null,
  created_at      timestamptz not null default now(),
  unique(fvs_padrao_id, numero_revisao)
);

-- Itens de verificação de uma FVS padrão (versionados por revisão)
create table fvs_padrao_itens (
  id              uuid primary key default uuid_generate_v4(),
  fvs_padrao_id   uuid not null references fvs_padrao(id) on delete cascade,
  revisao         integer not null, -- revisão à qual este item pertence
  ordem           integer not null default 0,
  titulo          text not null,
  metodo_verif    text, -- descrição do método de verificação
  tolerancia      text, -- ex: '± 5 mm', '1 cm / 5 m'
  created_at      timestamptz not null default now()
);
...
create table ambientes (
  id              uuid primary key default uuid_generate_v4(),
  obra_id         uuid not null references obras(id) on delete cascade,
  nome            text not null, -- ex: 'Apartamento 302', 'Fachada Norte'
  tipo            tipo_ambiente not null default 'interno',
  localizacao     text, -- ex: 'Piso 3', 'Torre A', 'Térreo'
  observacoes     text,
  ativo           boolean not null default true,
  created_at      timestamptz not null default now(),
  updated_at      timestamptz not null default now()
...
-- ============================================================
-- FVS PLANEJADAS (instâncias por ambiente)
-- Criadas pelo admin ao associar uma FVS padrão a um ambiente
-- ============================================================
create table fvs_planejadas (
  ...
  revisao_associada   integer not null, -- revisão da FVS Padrão quando foi associada
  subservico          text, -- ex: 'Piso', 'Paredes', 'Teto'
...
-- ============================================================
-- VERIFICAÇÕES (entradas no histórico de cada FVS planejada)
-- ============================================================
create table verificacoes (
  id                  uuid primary key default uuid_generate_v4(),
  fvs_planejada_id    uuid not null references fvs_planejadas(id) on delete cascade,
  numero_verif        integer not null, -- sequência dentro da FVS (1, 2, 3...)
  ...
  observacoes         text,
  assinatura_url      text, -- R2 URL do PNG da assinatura
  assinada_em         timestamptz,
  -- Sync metadata
  sync_id             uuid default uuid_generate_v4(), -- usado pelo PowerSync
...
-- ============================================================
-- ITENS DE VERIFICAÇÃO (resultado de cada item do checklist)
-- ============================================================
create table verificacao_itens (
  ...
  titulo              text not null, -- cópia do título no momento da verificação
  metodo_verif        text,          -- cópia do método
  tolerancia          text,          -- cópia da tolerância
  resultado           resultado_item not null default 'na',
  created_at          timestamptz not null default now()
);

-- ============================================================
-- FOTOS (de evidência — geral da verificação)
-- ============================================================
create table verificacao_fotos (
  id              uuid primary key default uuid_generate_v4(),
  verificacao_id  uuid not null references verificacoes(id) on delete cascade,
  r2_key          text not null, -- chave no Cloudflare R2
  r2_thumb_key    text,          -- thumbnail (400px)
  nome_arquivo    text,
...
-- Fotos específicas da não conformidade (evidência do problema)
...
-- Helper: retorna IDs de obras que o usuário logado pode acessar
...
-- Calcula o próximo número de verificação para uma FVS planejada
...
-- Atualiza status da FVS planejada após uma verificação
...
-- ============================================================
-- Executar após criar o primeiro usuário admin via Supabase Auth

-- Exemplo de seed (substituir UUIDs pelos reais):
/*
insert into empresas (id, nome, cnpj, municipio, uf) values
  ('11111111-1111-1111-1111-111111111111', 'Combrasen Engenharia Ltda', '12.345.678/0001-00', 'Brasilia', 'DF');

insert into usuarios (id, empresa_id, nome, cargo, perfil) values
  (auth.uid(), '11111111-1111-1111-1111-111111111111', 'José Elias', 'Sócio-Diretor', 'admin');
*/
